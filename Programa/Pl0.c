...
#define NOTOKENS 35   /*Numero de Tokens en el lenguaje                     */
...
enum simbolo tokpal[MAXPAL] = {si, sino, mientras, consttok, var, funcion, girarizquierda, parar, retroceder, apagate, comprobarcolor, comprobarparedarriba, comprobarparedabajo, comprobarparedderecha, comprobarparedizquierda};
...
main(int argc, char *argv[])
{
  ...
  else
  {
    ...
    init_set(tokinidecl);
    tokinidecl[consttok] = tokinidecl[var] = tokinidecl[funcion] = 1;

    init_set(tokiniinst);
    tokiniinst[llaveabierto] = tokiniinst[llavecerrado] = tokiniinst[si] = tokiniinst[mientras] = 1;

    init_set(tokinifact);
    tokinifact[identificador] = tokinifact[numero] = tokinifact[parentesisabireto] = 1;
...
void init_spec()
{
  ...
  espec[42] = por;
  espec[47] = barra;
  espec[40] = parentesisabireto;
  espec[41] = parentesiscerrado;
  espec[61] = igual;
  // espec[46]=punto;
  espec[44] = coma;
  espec[59] = puntoycoma;
...
void bloque(int toksig[])
{
  ...
  do
  {
    ...
    while (token == funcion)
    {
      ...
      if (token == puntoycoma)
      {
        ...
        setpaso[identificador] = setpaso[funcion] = 1; /*setpaso=tokiniinst+identificador+proctok*/
    ...
    copia_set(setpaso, tokiniinst);
    setpaso[identificador] = 1;     /*setpaso=tokiniinst+identificador*/
  ...
  copia_set(setpaso, toksig);
  setpaso[puntoycoma] = setpaso[parentesiscerrado] = 1; /*setpaso=puntoycoma+end+toksig*/
  instruccion(setpaso);
...
void declaracionconst(int *idat)
{
  ...
  {
    obtoken();
    if ((token == igual) || (token == asignacion))
    {
...
void instruccion(int toksig[])
{
  ...
  //   codigo[ic1].di = ic;
  // }
  else if (token == llaveabierto)
  {
    obtoken();

    copia_set(setpaso, toksig);
    setpaso[puntoycoma] = setpaso[llavecerrado] = 1; /*setpaso=puntoycoma+endtok+toksig*/
    instruccion(setpaso);

    while (token == puntoycoma || token == llaveabierto ||
           token == si || token == mientras)
    {
...
void factor(int toksig[])
{
  ...
  while (tokinifact[token] == 1)
  {
    ...
    else if (token == parentesisabireto)
    {
      /*solo se checa sintaxis pues los parentesis no interesan en RPN*/
      obtoken();

      copia_set(setpaso, toksig);
      setpaso[parentesiscerrado] = 1; /*setpaso=parenc+toksig*/
      expresion(setpaso);
    ...
    setpaso[parentesisabireto] = 1; /*setpaso=parena*/
...
void condicion(int toksig[])
{
  ...
  copia_set(setpaso, toksig);
  setpaso[igual] = setpaso[negacion] = setpaso[menor] = setpaso[mayor] = setpaso[menoroigual] = setpaso[mayoroigual] = 1;
  /*setpaso=igual+nig+mnr+myr+mei+mai+toksig*/

  expresion(setpaso);
  if ((token != igual) && (token != negacion) && (token != menor) &&
      (token != menoroigual) && (token != mayor) && (token != mayoroigual))
